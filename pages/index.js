...
export default function App() {
  ...
  const buttonStyle = {
    padding:"18px 50px",
    background:"linear-gradient(135deg, #FF6B9D 0%, #A18CD1 50%, #C44569 100%)", // PINK + PURPLE MIX
    color:"white",
    border:"none",
    borderRadius:"12px",
    fontSize:"18px",
  ...
  return (
    <div>
      <style>{`
       ...
       .btn:hover { transform: scale(1.05); box-shadow: 0 12px 30px rgba(161, 140, 209, 0.6); }
      ...
      <div style={{fontFamily:"'Segoe UI', Arial", background:"linear-gradient(to right,#FFE5EC,#E8DAFF)", minHeight:"100vh"}}>
        ...
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 40px",background:"white",boxShadow:"0 4px 15px rgba(0,0,0,0.1)",position:"sticky",top:0,zIndex:100}}>
          <h2 style={{color:"#A18CD1", fontSize:"28px", fontWeight:"900"}}>🎓 SRIT Mini Internship Portal</h2>
        ...
        <div style={{padding:"60px 40px", background:"white"}}>
          <h2 style={{textAlign:"center", background:"linear-gradient(135deg, #FF6B9D, #A18CD1)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", fontSize:"36px", marginBottom:"40px", fontWeight:"900"}}>👥 Meet Our Team</h2>
          <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(250px, 1fr))", gap:"30px", maxWidth:"1000px", margin:"0 auto"}}>
            {team.map((member, i) => (
              <div key={i} className="team-card" style={{background:"linear-gradient(135deg, #FF6B9D, #A18CD1, #C44569)", padding:"25px", borderRadius:"15px", textAlign:"center", color:"white", boxShadow:"0 8px 20px rgba(161,140,209,0.3)"}}>
        ...
        <footer style={{textAlign:"center", padding:"25px", background:"linear-gradient(135deg, #FF6B9D, #A18CD1)", color:"white", fontSize:"16px"}}>
